...
public class JoanaCollapsedVertex extends JoanaVertex implements CollapsedVertex {
	...
	/**
	 * Adds an edge which is connected to this collapsed vertex and represents a
	 * formerly present connection between a vertex in this collapsed vertex and
	 * one not being contained. This can be used to save old connections, which
	 * should be restored after expanding this vertex.
	 * 
	 * @param newEdge
	 *            the new edge
	 * @param oldEdge
	...
	/**
	 * Removes the connection of an edge connected to this collapsed vertex with
	 * the stored edge.
	 * 
	 * @param edge
	...
	public void removeModifiedEdge(JoanaEdge edge) {
		...
		// modifiedEdgeMap.get(edge).getSource().getID() + "->" +
		// modifiedEdgeMap.get(edge).getTarget().getID() );
